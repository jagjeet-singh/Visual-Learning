...
def showNN(pascal_classifier, fc7, pool5, eval_labels,eval_data, num_Images, image_list):
    ...
    for i in range(num_Images):
        ...
        plt.imshow(pool5Img)
        plt.xlabel("Pool5")
        fig.savefig('{0}_NN.png'.format(image_list[idx]))
        plt.show()
...
def main():
    ...
    pred_np = np.stack([p['probabilities'] for p in pred])

    AP = compute_map(eval_labels, pred_np, eval_weights, average=None)
    ...
    showNN(pascal_classifier, fc7, pool5, eval_labels,eval_data, 25, image_list)
